...
*  @copyright 2009-2017 Peter Sliacky
...
//        $this->context->smarty->assign("add_extra_div", ($opening_divs < $closing_divs)?$closing_divs-$opening_divs:false);
    ...
    {
        $this->context->smarty->assign('PS_GIFT_IMG', _PS_IMG_.Configuration::get('PS_GIFT_IMG'));

        $countdown_message = Module::getInstanceByName('carriercountdown')->getCountdownMessage();
        $this->context->smarty->assign('Message', $countdown_message);
...
//                            break;
                        ...
                        case 'saveDeliveryAddressInfo':       

                            $address_info = array();
                            $fields = array('email', 'company', 'vat_number', 'firstname', 'lastname', 'address1', 'address2', 'postcode', 'city', 'phone_mobile', 'id_country', 'id_state');
                            foreach ($fields as $field) {
                                $address_info[$field] = Tools::getValue($field);
                            }
                            
                            $this->context->cookie->{'cart_address_info_'.$this->context->cart->id} = Tools::jsonEncode($address_info);                            
            ...
            //throw new PrestaShopException('Empty cart - expired session.');
    ...
    if (count($addresses_c)) {
        ...
        if ($address_new_id > 0) {
            ...
            if ($this->isLogged) {
                ...
                $address_info = $this->context->cookie->{'cart_address_info_'.$this->context->cart->id};                
                if (!empty($address_info)) {
                    $this->context->smarty->assign('address_info', Tools::jsonDecode($address_info));
                }
            }
        ...
        if ($parse_payment_methods) {
         ...
         //print_r($payment_modules_list);
            ...
            foreach ($payment_modules_list as $payment_module) {
                if ($payment_module['module'] == 'free-order') {
                    $orig_hook_content = $return;
                } else {
                    $orig_hook_content = Hook::exec('displayPayment', array(), $payment_module['id_module']);
                }

                // Credit card module - special parsing
                // 1/ create new if-block, and output $orig_hook_content to console
                // 2/ take that to regex101.com and parse-out: image, label, action-link, cc form itself, including error <div>
                // 3/ add cc-form row (wrap in <div> with error) and place  Credit card form inside
                // 4/ edit modules/onepagecheckout/views/js/order-opc.js and update moveComplexPaymentForms()
                ...
                if ($payment_module['module'] == 'stripejs') {
                    ...
                    elseif ($whichMod->author == 'PrestaShop + Ollie McFarlane')
                    {
                        //echo $orig_hook_content; exit; 
                        $re = "/^(.*?)<a href=\".*?\"(.*?)<\\/a>(.*?)$/s"; 
                        $subst = "<div class=\"payment_module\"><a href=\"javascript:$('#stripe-form-container #stripe-payment-form').submit()\"><img src=\"modules/stripejs/img/stripe-logo.gif\" /><div id=\"stripe-form-container\"></div>     </a> $1 <div$2</div>$3</div>";
                        $subst_styles = "<style>
                        #stripe-payment-form .stripe-submit-button { display: none; }
                        .stripe-card-cvc-info {    font-style: italic;    display: inline-block;    cursor: pointer;    position: relative;    padding-bottom: 19px;}
                        .stripe-card-cvc-info div.cvc-info {    background-color: #BDE5F8;  border: #00529B solid 1px;  -moz-border-radius: 3px;    -webkit-border-radius: 3px; border-radius: 3px; color: #00529B!important;   display: none;  font-style: normal; padding: 8px;   position: absolute; right: -320px; top: -10px;  text-decoration: none;  width: 300px;}
                        .stripe-card-cvc-info:hover div.cvc-info { display: block; }
                        #paymentMethodsTable td {    vertical-align: baseline;}
                ...
                if ($payment_module['module'] == 'prestastripe') {
                    $re = '/^(.*?)<div class="payment_module".*?>.*?([^>]*?)<img [^<]*? src="([^>]*?logo\.png)".*?(<form.*?)<a href.*?>(.*?)<\/a>(.*?<\/form>\s*<div id="stripe-translations".*?<\/div>).*?(<script.*?<\/script>).*?$/s';
                    $subst = '<div class="payment_module"><a href="javascript:$(\'.stripe-submit-button\').click();"><img src="$3" />$2</label></td></tr><tr class="cc-form prestastripe-form-radio"><td></td><td colspan="2"><div id="prestastripe-form-container"></div><label></a></div>$4$6$7';
                    $subst_styles = "<style>
                ...
                if ($payment_module['module'] == 'mercadopago') {
                    $re = '/div class="mp-module"/s';
                    $subst = 'div class="payment_module mp-module"';
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content);
                }

                if ($payment_module['module'] == 'stripe_official') {
                    // Stripe official 1.3.2 by 202 commerce
                    // Remove any other anchor tags (CVC hint)
                    $orig_hook_content = preg_replace('/<a href=".*?"(.*?)<\/a>/s', '<div$1</div>', $orig_hook_content, 1);

                    $re = '/^.*?(<script.*?<\/script>).*?<div class="payment_module.*?>(.*?<h3.*?>(.*?)<\/h3>.*?src="([^"]*?powered_by_stripe.*?)".*?)(?:<\/div>\s*){3}(<div id="modal.*?)(<script.*?)$/s';
                    $subst = '<p class="payment_module"><a href="javascript:$(\'#stripe-payment-form\').submit();"><img src="$4" />$3</label></td></tr><tr class="cc-form stripe_official-form-radio"><td></td><td colspan="2"><div id="stripe_official-form-container"></div><label></a></p>$1$2$5$6';
                    $subst_styles = "<style>
                        #stripe-payment-form .stripe-submit-button, #stripe-payment-form-cc .stripe-submit-button-cc, img.powered_stripe, .stripe-card-cvc-info {
                            display: none;
                        }
                ...
                if ($payment_module['module'] == 'payubiz') {
                    $re = '/(<a href.*?onclick=\'.*?)return false;(\'>)(.*?)(<img.*?)(<\/a>)/s';
                    $subst = '$1$2$4$3$5';
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content);
                }

                if ($payment_module['module'] == 'ps_targetpay') {
                    $subst_styles = "<style>
                        #paymentMethodsTable [id^=targetpay_] input[type=submit] {
                            display: none;
                        }
                        .payment_description select {
                            max-width: 95%;
                        }
                        </style>";

                    $re = "/(<p class=.*?)(<img src=.*?method-([^\\/]+?)\\..*?\".*?)(<form)(.*?<\\/form>).*?(<\\/p>).*?/s";
                    $subst = "$1<a href=\"javascript:$('#targetpay_$3').find('input[type=submit]').click()\">$2<span id=\"targetpay_$3_form_anchor\"></span></a>$6$4 id=\"targetpay_$3\"$5";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content);
                }

                if ($payment_module['module'] == 'twocheckout') {
                    $re = '/^.*?(<script.*?)<div.*?(<p id="twocheckout-error.*?<\/p>).*?<h3.*?>.*?<img.*?src="(.*?)".*?>(.*?)<\/h3>.*?(<div.*?<\/div>.*?<form.*?<img src="(.*?twocheckout.*?)".*?<\/form>).*?(<script.*?<\/script>).*?$/s';
                    $subst = '<div class="payment_module"><a href="javascript:$(\'#twocheckoutCCForm\').submit();"><img src="$3" />$4<img src="$6" /></label></td></tr><tr class="cc-form twocheckout-form-radio"><td></td><td colspan="2"><div id="twocheckout-form-container"></div><label></a><div>$1$2$5</div>$7';
             
                    $subst_styles = "<style>
                        #twocheckoutCCForm .button, #twocheckoutCCForm .block-right img, #twocheckoutCCForm .block-left br {
                            display: none;
                        }
                        div#twocheckout_error_creditcard {
                            color: red;
                            font-size: 1.2em;
                            text-align: center;
                ...
                if ($payment_module['module'] == 'brinkscheckout') {

                    $re = "/(.*?)<div.*?class=\"payment_module\">\\s*<h3.*?\\/>(.*?)\\s+<\\/h3>(.*?)<div class=\"block-right\">.*?<img src=\"(.*?)\".*?<\\/div>.*?(<\\/form>).*?(<script.*)/s";
                    $subst = "<div class=\"payment_module brinks-checkout\"><a href=\"javascript:$('#twocheckoutCCForm').submit()\"><img src=\"$4\" />$2</label></td></tr><tr class=\"cc-form brinkscheckout-form-radio\"><td></td><td colspan=\"2\"><div id=\"brinkscheckout-form-container\"></div><label></a> </div>$1<div>$3$5</div>$6";

                    $subst_styles = "<style>
                        #paymentMethodsTable .payment_description img { max-height: none; }
                        #twocheckoutCCForm #submit_payment { display: none; }
                    </style>";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }

                if (Tools::substr($payment_module['module'], 0, Tools::strlen('collector')) === 'collector') {
                    
                    $re = "/.*?<noscript>(.*?id_module=(\\d+)).*?style=\"(.*?)\"(.*?)<\\/noscript>.*/s";
                    $subst = '<style>p.payment_module a.collector-$2 {$3}</style>$1" class="collector-$2"$4';
 
                    $subst_styles = "";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }

                if (Tools::substr($payment_module['module'], 0, Tools::strlen('beanstream')) === 'beanstream') {
                    
                    $re = "/^.*?(.*?id_module=(\\d+)).*?style=\"(.*?)\"(.*)$/s"; 
                    $subst = '<style>p.payment_module a.beanstrem-$2 {$3}</style>$1" class="beanstrem-$2"$4';
 
                    $subst_styles = "";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }

                if ($payment_module['module'] == 'dotpay') {
                    
                    $re = '/data-type="dotpay_payment_link"/';
                    $subst = 'href="javascript:dotpay_payment_link_handler(null)"';
                ...
                if ($payment_module['module'] == 'paypalusa') {
                    ...
                    $re = '/^.*?<p class.*?>(.*?paypal-express-checkout-form.*?expresscheckout.*?src="(.*?)".*?)<\/p>(.*?)$/s';
                    $subst = '<p class="payment_module"><a href="javascript:$(\'#paypal-express-checkout-form\').submit()"><img src="modules/onepagecheckout/views/img/transparent.gif" /><img src="$2" /></a></p>$1';
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                ...
                if ($payment_module['module'] == 'paypalmx') {
                    $re = "/.*?form id=\"(.*?)\".*?input.*?image.*?src=\"(.*?)\"/s"; 
                    $subst = "<p class=\"payment_module\"><a href=\"javascript:$('#$1').submit()\"><img src=\"modules/onepagecheckout/views/img/transparent.gif\" /><img src=\"$2\" /></a></p>\n\n$0";
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                    
                }
                if ($payment_module['module'] == 'paypalwithfee') {
                    $re = "/id=\"paypal_process_payment_\"/"; 
                    $subst = "";
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                }

                if ($payment_module['module'] == 'pronesis_bancasella') {
                    $re = "/^(.*?)\\$\\(document\\).ready.*?click\\(function\\(\\)\\s*{(.*?return false;).*?(<\\/script>.*?<a href=\").*?(\".*?)$/s"; 
                    $subst = "$1function banca_sella_confirm() { $2 } $3javascript:banca_sella_confirm();$4";
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                    
                }
                
                if ($payment_module['module'] == 'obsredsys') {
                    $re = '/^(.*?<a href.*?)style="(.*?)"(.*?)$/s';
                    $subst = '<style>.payment_module a.obsredsys-link {$2}</style>$1 class="obsredsys-link"$3';
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }

                if ($payment_module['module'] == 'paypal') {
                    $re = '/^(.*?)<a href="javascript:void\(0\)".*?id="paypal_process_payment"(.*?form id="(.*?)"(.*?))$/s';
                    $subst = '$1<a href="javascript:$(\'#$3\').submit()"$2';
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                

                    // replace very wide horizontal logos with default one: 
                    $re = '/logos\/.{1,3}horizontal_solution.*?"/s';
                    $subst = 'paypal.gif"';
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                }

                if ($payment_module['module'] == 'npaypalpro') {
                    //echo $orig_hook_content; exit;
                    $re = "/^(.*?)<div class.*?<h3.*?\\/>(.*?)(<img.*?>).*?(<div.*?)<a href.*?<\\/a>(.*?paypalpro-card-del-error.*?<\\/div>).*?(<script.*?)$/s"; 
                    $subst = "<div class=\"payment_module\"><a href=\"javascript: $('#paypalpro-payment-form').submit();\"><img src=\"/modules/onepagecheckout/views/img/transparent.gif\" />$2$3</label></td></tr><tr class=\"cc-form npaypalpro-form-radio\"><td></td><td colspan=\"2\"><div id=\"npaypalpro-form-container\"></div><label></a> </div>$1$4$5";
                    $subst_styles = "<style>
                        #paypalpro-payment-form button.paypalpro-submit-button {
                            display: none;
                        }
                        #paypalpro-payment-form label {
                            width: auto;
                        }
                        #npaypalpro-form-container p > span {
                            max-width: 176px;
                        }
                        input.paypalpro-lastname, input.paypalpro-firstname {
                            max-width: 170px;
                        }
                        #paypalpro-payment-form label {
                            padding: 0 0 5px;
                ...
                if ($payment_module['module'] == 'przelewy24') {
                    $re = "/(<div class.*?title=\".*?([^ ]*?)\".*?)style=\"(.*?)\"(.*?<\\/div>)/s";
                    $subst = '<style>a.przelewy24.$2 {$3}</style>$1 class="przelewy24 $2"$4';
                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content);
                }
                ...
                if ($payment_module['module'] == 'authorizedotnet') {
                    $re = "/.*(<div id=\"adn_payment\".*?)<h2 class=\"title_accept\">(.*?)<\\/h2>.*?(<div class=\"accept_cards\".*?<\\/div>)(.*?<\\/form>).*?(<script.*?<\\/script>).*/s";
                    $subst = "<div class=\"payment_module\"><a href=\"javascript:paymentModuleAND.send(document.adn_form)\"><img src=\"modules/onepagecheckout/views/img/auth_secure_logo.gif\" />$2$3</label></td></tr><tr class=\"cc-form authorizedotnet-form-radio\"><td></td><td colspan=\"2\"><div id=\"authorizedotnet-form-container\"></div><label></a> </div>$1$4</div>$5";

                    $subst_styles = "<style>
                        #adn_submit {display: none}
                        </style>";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }
                if ($payment_module['module'] == 'nmi') {
                    $re = '/^(.*?)<p class=\"payment_module\".*?(<form.*?>).*?<a.*?>(.*?)<\/a>.*?(<img.*?logo.*?>).*?(<input.*?)(<input type="button".*?>).*?(<script.*?<\/script>).*$/s';
                    $subst = '<div class="payment_module"><a href="javascript:$(\'#nmisubmit\').click()">$4$3</label></td></tr><tr class="cc-form nmi-form-radio"><td></td><td colspan="2"><div id="nmi-form-container"></div><label></a> </div>$2$5$6</form>$7';
                ...
                if ($payment_module['module'] == 'firstdata') {
                    $re = "/^(.*?<\\/script>).*?<img src=\"(.*?)\".*?<h3.*?><img.*?>(.*?)<\\/h3>(<form.*?)<a href.*?<\\/a>(.*?<\\/form>).*?$/"; 
                    $subst = "<div class=\"payment_module\"><a href=\"javascript:$('#firstdata_form').submit()\"><img src=\"$2\" />$3</label></td></tr><tr class=\"cc-form firstdata-form-radio\"><td></td><td colspan=\"2\"><div id=\"firstdata-form-container\"></div><label></a> </div>$1</div>$4$5";
                    $subst_styles="<style>#firstdata_submit {display: none;}</style>";
                    $orig_hook_content = preg_replace($re, $subst_styles.$subst, $orig_hook_content, 1);
                }
                if ($payment_module['module'] == 'payfortfort') {
                    $re = '/^.*?<a class=.*?url\((.*?)\).*?>\s*(.*?)\s*<\/a>.*?(<form.*?<\/form>).*?(<script.*?script>).*?$/s';
                    $subst = '<div class="payment_module"><a href="javascript:payfortFortMerchantPage2.submitMerchantPage()"><img src="$1" />$2</label></td></tr><tr class="cc-form payfortfort-form-radio"><td></td><td colspan="2"><div id="payfortfort-form-container"></div><label></a></div>$3$4';
                    $subst_styles="<style>
                                    form#frm_payfortfort_merchantpage2 p.clearfix {
                                        display: none;
                                    }
                                    select#payfort_fort_expiry_month {
                                        margin-left: -8px;
                                        width: 60px;
                                    }
                                    select#payfort_fort_expiry_year {
                                        margin-left: 38px;
                                        width: 100px;
                                    }
                                   </style>";
                ...
                if ($payment_module['module'] == 'winbnk') {
                    $re = "/^(<p.*?>).*?(<img src.*?\\/>).*?<span>(.*?)<\\/span>(.*?)$/s"; 
                    $subst = "$1<a href=\"javascript:$('form[name=winbnk_confirmation]').submit()\">$2$3</a>$4";

                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                }

                if ($payment_module['module'] == 'epay') {
                    $re = "/https:\\/\\/relay.ditonlinebetalingssystem.dk\\/integration\\/paymentlogos\\/Images\\/uk.gif/"; 
                    $subst = "https://d25dqh6gpkyuw6.cloudfront.net/epaylogos/uk.gif";

                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content, 1);
                }

                // https://regex101.com/r/q0XB9U/1
                // https://regex101.com/r/IHQkrK/1
                if ($payment_module['module'] == 'payline') {
                    // Simple form submit options
                    $re = '/<div class="paylineSpecificBlock">.*?<img src="(.*?)".*?\/>.*?<p.*?payline_payment.*?data-card-type="(.*?)".*?contract="(.*?)".*?data-mode="(.*?)">.*?<img src="(.*?)".*?\/>.*?<span>(.*?)<\/span>.*?(\s*<\/div>){3}/s';
                    $subst = '<p class="payment_module"><a href="javascript:$(\'#contractNumber\').val(\'\\3\');$(\'#mode\').val(\'\\4\');$(\'#type\').val(\'\\2\');$(\'#WebPaymentPayline\').submit();"><img src="\\1" /><img src="\\5" /><span class="contract-name">\\6</span></a></p>';

                    $orig_hook_content = preg_replace($re, $subst, $orig_hook_content);

                    // CC form option
                    $re = '/<img src="(.*?)".*?(<form.*?name="directPaymentPayline".*?>.*?<input type="radio")(.*?<img src="(.*?)".*? \/>.*?<\/form>)/s';
                    $subst = '<div class="payment_module"><a href="javascript:$(\'#directPaymentPayline button[type=submit]\').click();"><img src="$1" /><img src="$4" /></label></td></tr><tr class="cc-form payline-form-radio"><td></td><td colspan="2"><div id="payline-form-container"></div><label></a> </div>$2 checked="checked" $3</div>';
                    $subst_styles = "<style>
                        .contract-name {padding-left: 10px;}
                        #directPaymentPayline button[type=submit] {
                            display: none;
                        }
                        </style>";
                    $orig_hook_content = preg_replace($re, $subst.$subst_styles, $orig_hook_content);
                }

                if ($payment_module['module'] == 'pireospay') { 
                    // Continue only when installments are enabled: 
                    if (preg_match('/id="pireospay_installments"/', $orig_hook_content))
                    {
                        $re = '/^.*?(<form.*>).*?(<img.*?src=".*?".*?>)(.*?)<br.*?>(.*?)<button(.*?)$/s';
                        $subst = '<div class="payment_module"><a href="javascript:$(\'.pireospay_choice\').submit();">$2$3</label></td></tr><tr class="cc-form pireospay-form-radio"><td></td><td colspan="2"><div id="pireospay-form-container"></div><label></a></div>$1$4<input type="hidden" name="pireospay_payment" value="1" /></form>';

                        $subst_styles = "<style>
                            button[name=pireospay_payment] {
                                display: none;
                            }
                            </style>";
                        $orig_hook_content = preg_replace($re, $subst.$subst_styles, $orig_hook_content);
