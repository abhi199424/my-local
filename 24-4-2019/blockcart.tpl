...
<div class="col-sm-1 clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if} cart-section">
{/if}
	<div class="shopping_cart" id="header_shopping_cart">
		{*<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">*}
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}?cart-only" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			...
			<span class="ajax_cart_no_product unvisible">{l s='(empty)' mod='blockcart'}</span>
		...
		<div class="cart_data_boom">
		{if $free_shipping > 0 || true}
			{assign var='carttotal' value=$cart->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING)}
			{assign var='freevalue' value=Configuration::get('PS_FREE_SHIPPING',null,null,Context::getContext()->shop->id)}
			<input type="hidden" class="topcarttexttopValue" value="{l s='Free shipping in' mod='blockcart'}" />
			<input type="hidden" class="topcarttextValue" value="{l s='Free shipping' mod='blockcart'}" />
			{if $carttotal < $freevalue}
				{assign var='minusvalue' value=$freevalue - $carttotal}
				<a class="topcarttext" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
				  {l s='Forventet levering' mod='blockcart'} {convertPrice price=$minusvalue|floatval}
				</a>
			{else}
				<a class="topcarttext" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
...
<div class="top-section">
	<div class="top-custom-links">{hook h=TopCustomLink}<div class="clear"></div></div>
	<div class="top-search-area">{hook h=Top_search}<div class="clear"></div></div>
</div>
...
{if !$PS_CATALOG_MODE && $active_overlay == 1}
		<div id="layer_cart">
		<div class="clearfix">
			<div class="layer_cart_product_custom">
				{*<h4>{l s='The product is added to your basket' mod='blockcart'}</h4>*}
				<div class="layer_cart_product_div">
                    {*<h2>{l s="Other customers did also order:" mod='blockcart'}</h2>*}
								<div class="layer_cart_product col-xs-12 col-md-6">
				<span class="cross" title="{l s='Close window' mod='blockcart'}"></span>
				<span class="title">
					{l s='THE PRODUCT IS ADDED' mod='blockcart'}
				</span>
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				<span class="title">
					...
					<span class="ajax_cart_product_txt_s {if $cart_qties < 2} unvisible{/if}">
						{l s='GO TO BASKET' mod='blockcart' sprintf=[$cart_qties] tags=['<span class="ajax_cart_quantity">']}  <i class="icon-shopping-cart"></i>
					</span>
					<a class=" continue close">×</a>
					<!-- Singular Case [both cases are needed because page may be updated in Javascript] -->
					<span class="ajax_cart_product_txt {if $cart_qties > 1} unvisible{/if}">
						{l s='GO TO BASKET' mod='blockcart'}  <i class="icon-shopping-cart"></i>
					</span>
				</span>
				...
				<table>
					...
					<tbody>
						<tr>
							<td>
								<span id="layer_cart_product_quantity" class="cartpopupitem"></span> <span class="cartpopupitem">{l s='Total products' mod='blockcart'}</span>
							</td>
							<td class="amountdisplay">
								<span id="layer_cart_product_price" class="cartpopupitem"></span>
							</td>
						</tr>
						<tr>
							<td>
								<span class="cartpopupitem">{l s='i alt' mod='blockcart'}</span>
							</td>
							<td class="totalamountdisplay">
								<span class="ajax_block_cart_total cartpopupitem">
									{if $cart_qties > 0}
										{if $priceDisplay == 1}
				...
				*}
				<div class="button-container">
					<a class="continue btn btn-default button btcontinue" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
							{l s='SHOP FURTHER' mod='blockcart'}
						</span>
					</a>
					<a class="btn btn-default button btcontinue popupbtn1"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}">
						<span>
							{l s='GO TO CART' mod='blockcart'}
						</span>
					</a>
				</div>
				<div class="row cart_free_shipping_popup">
					<input type="hidden" class="achievefreedelivery" value="{l s='You have achieved free delivery!' mod='blockcart'}" />
					<input type="hidden" class="getfreedelivery" value="{l s='Get FREE delivery. Buy for ' mod='blockcart'}" />
					<input type="hidden" class="getfreedeliverymore" value="{l s='more' mod='blockcart'}" />
					<div class="cart_free_shipping_popup_text">
					</div>
				</div>
			</div>
			<span class="title slidertitle">
				{l s='OTHERS ALSO PURCHASED' mod='blockcart'}
			</span>
			<div class="row cart_layer_product">
			</div>
		</div>
	</div>
				{*<div class="button-container">
									<span class="continue btn btn-default button exclusive-medium " title="{l s='Continue shopping' mod='blockcart'}">
										<span>
				                            ...
				                            <img src="{$img_ps_dir}check_arr1.jpg" alt="<"/>
									...
									<a class="btn btn-default button button-medium"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
										<span>
				                            <img src="{$img_ps_dir}check_arr2.jpg" alt=">"/>
											{l s='Go to basket' mod='blockcart'}
										</span>
									</a>
								</div>*}
			</div>
